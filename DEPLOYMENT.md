...
Esto garantiza que la base de datos (`/app/data/pim.db`) no se pierda al reiniciar o redesplegar.
...
Asegúrate de haber configurado correctamente el volumen persistente en `/app/data`
...
docker cp <container-id>:/app/data/pim.db ./backup-$(date +%Y%m%d).db
```
...
docker cp ./backup-20260325.db <container-id>:/app/data/pim.db
